...
class UserRegistrationSerializer(serializers.ModelSerializer):
    username_validator = RegexValidator(regex=r'^\w[\w.@+-]*$', message="Имя пользователя должно соответствовать регулярному выражению: ^[w.@+-]+$")
    username = serializers.CharField(required=True, min_length=4, max_length=150, validators=[username_validator])
...
class RecipeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if 'ingredients' not in self.initial_data:
            raise serializers.ValidationError({"error": "Field 'ingredients' is required."})

        if not self.initial_data['ingredients']:
            raise serializers.ValidationError({"error": "At least one ingredient is required."})

        ingredient_ids = [ingredient['id'] for ingredient in self.initial_data['ingredients']]
        ...
        if 'tags' not in attrs:
            raise serializers.ValidationError({"error": "Field 'tags' is required."})

        # Проверка наличия тегов
        if not attrs['tags']:
            raise serializers.ValidationError({"error": "Field 'tags' cannot be empty."})

        # Проверка дублирования тегов
        tags_ids = [tag.id for tag in attrs['tags']]
        if len(tags_ids) != len(set(tags_ids)):
            raise serializers.ValidationError({"error": "Duplicate tags are not allowed."})

        # Проверка наличия поля 'cooking_time'
        if 'cooking_time' not in attrs:
            raise serializers.ValidationError({"error": "Field 'cooking_time' is required."})

        # Проверка времени готовки
        cooking_time = attrs['cooking_time']
        if cooking_time == "":
            raise serializers.ValidationError({"error": "Cooking time must be specified."})
        if cooking_time < 1:
            raise serializers.ValidationError({"error": "Cooking time cannot be less than 1 minute."})

        if 'image' not in attrs:
            raise serializers.ValidationError({"error": "Field 'image' is required."})
